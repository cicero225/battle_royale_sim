...
# necessary for some tests. This kind of mixing in tests is bad, but otherwise it's a huge amount of work to fake these classes.
from Events import *
...
class ContestantTest(object):
    ...
    def __init__(self):  # This incidentally tests the constructor, but will probably be useful for other things
        self.settings = {
            ...
            # Percent range over which the base weights of events varies from json settings
            "eventRandomness": 0.5,
            # How much stats influence event weightings, calculated as (1+influence)^((stat-5)*eventInfluenceLevel)
            "statInfluence": 0.3,
            # How much objects in inventory affect events. The default 1 uses the base stats.
            "objectInfluence": 1,
            # How much relationships affect event chance, calculated as (1+influence)^(relationship level*eventInfluenceLevel)
            "relationInfluence": 0.3
        }
        self.name = "FakeContestant"
        self.inDict = collections.OrderedDict()
        self.inDict["imageFile"] = "dummy.jpg"
        self.inDict["stats"] = {"wideness": 6,
                                "suffering": 4, "potential": 10, "english": 0}
    ...
    def testRandomContestant(self):
        Contestant.makeRandomContestant(
            "Rando", "dummy.jpg", self.contestant.stats, self.settings)
    ...
    def testItemHandling(self):
        ...
        # Cannot remove twice...
        assert not self.contestant.removeItem(dummyItem)
        # TODO: when dummy event set is made, test that EventModifiers are correct
        self.refreshTest()
