...
def func(self, mainActor, state=None, participants=None, victims=None, sponsors=None):
    ...
    potentialContestants = DictToOrderedDict({x.name: (abs(state["allRelationships"].friendships[mainActor.name][x.name]) + 2 * abs(
        state["allRelationships"].loveships[mainActor.name][x.name])) for x in state["contestants"].values() if x.alive and x != mainActor})
