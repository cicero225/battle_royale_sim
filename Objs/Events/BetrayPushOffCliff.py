...
def func(self, mainActor, state=None, participants=None, victims=None, sponsors=None):
    desc = mainActor.name + ' invited ' + str(victims[0]) + ' up to the top of a gorge to admire the view, but betrayed ' + Event.parseGenderObject(
        victims[0]) + ', pushing ' + Event.parseGenderObject(victims[0]) + ' off the cliff to ' + Event.parseGenderPossessive(victims[0]) + ' death.'
    ...
    if mainActor.stats["stability"] < 3:
        desc += ' ' + Event.parseGenderSubject(mainActor).capitalize() + ' smiled as ' + Event.parseGenderSubject(
            mainActor) + ' watched ' + Event.parseGenderObject(victims[0]) + ' fall.'
