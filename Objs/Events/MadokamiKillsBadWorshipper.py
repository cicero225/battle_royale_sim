...
def func(self, mainActor, state=None, participants=None, victims=None, sponsors=None):
    ...
    if success:
        ...
        desc = sponsors[0].name+' struck down '+mainActor.name + " for "+Event.parseGenderPossessive(mainActor)+" blasphemous beliefs."
        tempList = [sponsors[0], mainActor]
        del state["events"]["AWorshipsB"].eventStore[mainActor.name]
        deadList = [mainActor.name]
    else:
        desc = sponsors[0].name+' tried to strike down '+mainActor.name + " for "+Event.parseGenderPossessive(mainActor)+" blasphemous beliefs, but Akuma Homura was able to intervene successfully."
