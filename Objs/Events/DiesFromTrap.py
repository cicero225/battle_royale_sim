...
TRAP_RESULT = [' fell into a spike trap set by SOURCE, impaling VICTRE on a wooden spike.', ' was struck by a hidden dart trap set by SOURCE, causing VICTIM to die slowly and painfully.',
               ' fell into a fall trap set by SOURCE, causing VICTIM to fall off a cliff and crack open VICTPOS skull.']
TRAP_RESULT_SELF = [' fell into SOURCE own spike trap, impaling VICTRE on a wooden spike.', ' was struck SOURCE own dart trap, causing VICTIM to die slowly and painfully.',
                    ' fell into SOURCE own fall trap, causing VICTIM to fall off a cliff and crack open VICTPOS skull.']


def setupDiesFromTrap(state):
    state["events"]["DiesFromTrap"].eventStore["trapCounter"] = DictToOrderedDict(
        {0: 0, 1: 0, 2: 0})  # Stores existing traps
    # Accounts for traps that people themselves made (to abstract away the fact that people ideally don't fall into their own traps)
    state["events"]["DiesFromTrap"].eventStore["trapMakerCounter"] = collections.OrderedDict()
    ...
    if event.name == "DiesFromTrap":
        ...
        for key in thisTrapDict:
            if str(actor) in event.eventStore["trapMakerCounter"]:
                thisTrapDict[key] -= event.eventStore["trapMakerCounter"][str(
                    actor)][key]
...
def func(self, mainActor, state=None, participants=None, victims=None, sponsors=None):
    ...
    if trapSource == str(mainActor):
        desc = "Delirious and confused, " + str(mainActor) + TRAP_RESULT_SELF[chosen].replace("VICTIM", Event.parseGenderObject(mainActor)).replace(
            "VICTRE", Event.parseGenderReflexive(mainActor)).replace("SOURCE", Event.parseGenderPossessive(mainActor)).replace("VICTPOS", Event.parseGenderPossessive(mainActor))
        descList = [mainActor]
    else:
        desc = str(mainActor) + TRAP_RESULT[chosen].replace("VICTIM", Event.parseGenderObject(mainActor)).replace(
    ...
    return (desc, descList, [str(mainActor)], {str(mainActor): trapSource}, [mainActor])
