...
def setupDiesFromTrap(state):
    state["events"]["DiesFromTrap"].eventStore["trapCounter"] = DictToOrderedDict({0: 0, 1: 0, 2: 0})  # Stores existing traps
    state["events"]["DiesFromTrap"].eventStore["trapMakerCounter"] = collections.OrderedDict()  # Accounts for traps that people themselves made (to abstract away the fact that people ideally don't fall into their own traps)
...
def func(self, mainActor, state=None, participants=None, victims=None, sponsors=None):
    ...
    trapSourceDict = ArenaUtils.DictToOrderedDict({key: self.eventStore["trapMakerCounter"][key][chosen] for key in self.eventStore["trapMakerCounter"]})
