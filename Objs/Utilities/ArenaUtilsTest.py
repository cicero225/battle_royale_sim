...
class ArenaUtilsTest(object):
    ...
    settings = collections.OrderedDict(); # This might have to be more elaborate for some of the other tests
    ...
    def TestWeightedRandom(self):
        ...
        for x in inDict:
            assert x in test4
        resultDict = {"homu": [0, 0, 0], "mado": [0, 0, 0], "saya": [0, 0, 0], "koko": [0, 0, 0]}
        for x in range(400):
            for y in range(3):
        ...
        assert abs(resultDict["homu"][0]/400 - 1/3) < 0.075 # ~99% confidence here
